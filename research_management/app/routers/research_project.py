...
@router.post("", status_code=status.HTTP_201_CREATED, summary="Tạo đề tài nghiên cứu", description="Tạo một đề tài nghiên cứu mới")
...
@router.get("", response_model=list[ProjectResponse], summary="Lấy danh sách đề tài", description="Lấy danh sách các đề tài nghiên cứu mà người dùng hiện tại tham gia")
def get_projects(user: User = Depends(get_current_user), db: Session=Depends(get_db), keyword: str | None = None):
    projetcs = get_projects_service(user, db, keyword)
    if not projetcs:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Không tìm thấy dự án nào")
    return projetcs


@router.get("/{id}", summary="Lấy chi tiết đề tài", description="Lấy thông tin chi tiết của một đề tài nghiên cứu")
def get_project(id: int, user: User=Depends(get_current_user), db: Session=Depends(get_db)):
    return get_project_service(id, user, db)


@router.patch("/{id}", summary="Cập nhật đề tài", description="Cập nhật thông tin của một đề tài nghiên cứu")
...
@router.delete("/{id}", summary="Xóa đề tài", description="Xóa một đề tài nghiên cứu")
