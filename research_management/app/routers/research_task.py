...
@router.post("/research-projects/{id}/research-tasks", status_code=status.HTTP_201_CREATED, summary="Tạo nhiệm vụ")
...
@router.get("/research-projects/{id}/research-tasks", response_model=list[TaskResponse], summary="Lấy danh sách nhiệm vụ")
def get_tasks(
    project_id: int, 
...
@router.get("/research-tasks/{id}", response_model=TaskResponse, summary="Lấy chi tiết nhệm vụ")
def get_detail_task(id: int, user: User=Depends(get_current_user), db: Session=Depends(get_db)):
    return get_task_by_id(id, user, db)


@router.patch("/research-tasks/{id}", summary="Cập nhật nhiệm vụ")
...
@router.delete("/research-tasks/{id}", summary="Xóa nhiệm vụ")
