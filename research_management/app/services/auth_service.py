...
from app.utils.validate import validate_pass, validate_space
...
def register_service(user: RegisterRequest, db: Session):
    try:
        ...
        hashed = hash_password(validate_pass(user.password))
        new_user = User(
            email = validate_space(user.email),
            password_hash = hashed,
            full_name = validate_space(user.full_name)
        )
...
# def login_service(data: OAuth2PasswordRequestForm, db: Session):
