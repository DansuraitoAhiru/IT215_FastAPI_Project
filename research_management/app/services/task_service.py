...
def get_all_tasks(
    ...
    if asignee_id:
        query = query.filter(ResearchTask.assignee_id == asignee_id)
    if keyword:
        query = query.filter(ResearchTask.title.ilike(f"%{keyword}%"))
    if sort == "created_at":
        query = query.order_by(ResearchTask.created_at)
    else:
        query = query.order_by(ResearchTask.due_date)
